...
function read(summary:string,tag:string){return{summary,tags:[tag],security:[{workspaceAuth:[]}],responses:{"200":{description:"Operação concluída"},...errors}}}
function write(summary:string,tag:string,permission:string){return{summary,tags:[tag],security:[{workspaceAuth:[]}],"x-permission":permission,requestBody:{required:true,content:{"application/json":{schema:{$ref:"#/components/schemas/Command"}}}},responses:{"200":{description:"Operação concluída"},"201":{description:"Registo criado ou transição executada"},...errors}}}
export const openApiDocument={
 ...
 components:{securitySchemes:{workspaceAuth:{type:"apiKey",in:"header",name:"oai-authenticated-user-email",description:"Header de identidade injetado pela plataforma; nunca fornecido pelo cliente."}},parameters:{Period:{name:"period",in:"query",schema:{type:"string",pattern:"^\\d{4}-(0[1-9]|1[0-2])$",example:"2026-08"}},Currency:{name:"currency",in:"query",schema:{type:"string",pattern:"^[A-Z]{3}$",example:"AOA"}},Version:{name:"version",in:"query",schema:{type:"string"}}},schemas:{Command:{type:"object",required:["type"],properties:{type:{type:"string",description:"Discriminador explícito do comando"}},additionalProperties:true},Error:{type:"object",required:["error"],properties:{error:{type:"string"}}}}}
} as const;
