...
test("attendance preserves entries and requires governed timesheet decisions",()=>{
 const db=migratedDatabase();db.exec("INSERT INTO tenants VALUES ('t1','now','Tenant','tenant','Ativo'),('t2','now','Outro','outro','Ativo'); INSERT INTO organizations VALUES ('o1','t1','now','ROOT','Org','Empresa','AOA','Ativa'),('o2','t2','now','ROOT','Outra','Empresa','AOA','Ativa'); INSERT INTO employees VALUES ('e1','t1','now','001','Ana','Silva','o1','Analista','2026-01-01','Ativo'),('e2','t2','now','002','Eva','Lima','o2','Analista','2026-01-01','Ativo'); INSERT INTO attendance_shifts VALUES ('s1','t1','DIA','Diurno',480,1020,60,480,'Ativo','rh@test','now')");
 assert.throws(()=>db.exec("INSERT INTO attendance_assignments VALUES ('a0','t1','e2','s1','2026-01-01',NULL,'Ativa','rh@test','now')"),/attendance assignment outside tenant/);
 db.exec("INSERT INTO attendance_assignments VALUES ('a1','t1','e1','s1','2026-01-01',NULL,'Ativa','rh@test','now'); INSERT INTO attendance_entries VALUES ('x1','t1','e1','2026-08-20','08:00',NULL,'Manual',NULL,480,NULL,NULL,NULL,'ana@test','now'); UPDATE attendance_entries SET clock_out='18:00',worked_minutes=600,overtime_minutes=120,exception_code='EXTRA' WHERE id='x1'; INSERT INTO attendance_timesheets VALUES ('tsh1','t1','e1','2026-08','Rascunho',600,480,120,1,'hash',NULL,NULL,NULL,NULL,NULL,'now')");
 assert.throws(()=>db.exec("UPDATE attendance_entries SET worked_minutes=601 WHERE id='x1'"),/invalid attendance entry close/);db.exec("UPDATE attendance_timesheets SET status='Submetido',submitted_by='ana@test',submitted_at='later' WHERE id='tsh1'");assert.throws(()=>db.exec("UPDATE attendance_timesheets SET status='Rascunho' WHERE id='tsh1'"),/invalid attendance timesheet transition/);db.exec("UPDATE attendance_timesheets SET status='Aprovado',decided_by='manager@test',decided_at='done' WHERE id='tsh1'");assert.equal(db.prepare("SELECT overtime_minutes FROM attendance_timesheets WHERE id='tsh1'").get().overtime_minutes,120);assert.throws(()=>db.exec("DELETE FROM attendance_timesheets WHERE id='tsh1'"),/attendance timesheets cannot be deleted/);db.close();
});

test("payroll loans preserve principal and settle only through governed installments",()=>{
 const db=migratedDatabase();db.exec("INSERT INTO tenants VALUES ('t1','now','Tenant','tenant','Ativo'),('t2','now','Outro','outro','Ativo'); INSERT INTO organizations VALUES ('o1','t1','now','ROOT','Org','Empresa','AOA','Ativa'),('o2','t2','now','ROOT','Outra','Empresa','AOA','Ativa'); INSERT INTO employees VALUES ('e1','t1','now','001','Ana','Silva','o1','Analista','2026-01-01','Ativo'),('e2','t2','now','002','Eva','Lima','o2','Analista','2026-01-01','Ativo')");assert.throws(()=>db.exec("INSERT INTO payroll_loans VALUES ('bad','t1','e2','Empréstimo','LN-X','AOA',100,1,'2026-09','Teste','Pendente',100,'ana@test','now',NULL,NULL,NULL,NULL)"),/payroll loan outside tenant/);
 db.exec("INSERT INTO payroll_loans VALUES ('l1','t1','e1','Empréstimo','LN-1','AOA',10001,2,'2026-09','Educação','Pendente',10001,'ana@test','now',NULL,NULL,NULL,NULL); UPDATE payroll_loans SET status='Aprovado',decided_by='rh@test',decided_at='later' WHERE id='l1'; INSERT INTO payroll_loan_installments VALUES ('i1','t1','l1',1,'2026-09',5001,'Programada',NULL),('i2','t1','l1',2,'2026-10',5000,'Programada',NULL)");assert.equal(db.prepare("SELECT SUM(amount_minor) total FROM payroll_loan_installments WHERE loan_id='l1'").get().total,10001);assert.throws(()=>db.exec("UPDATE payroll_loan_installments SET amount_minor=1 WHERE id='i1'"),/invalid payroll installment transition/);db.exec("UPDATE payroll_loans SET outstanding_minor=5000 WHERE id='l1'; UPDATE payroll_loan_installments SET status='Liquidada',paid_at='paid' WHERE id='i1'");assert.throws(()=>db.exec("DELETE FROM payroll_loans WHERE id='l1'"),/payroll loans cannot be deleted/);db.close();
});

test("retroactive adjustments are immutable and never rewrite source payroll",()=>{
 const db=migratedDatabase();db.exec("INSERT INTO tenants VALUES ('t1','now','Tenant','tenant','Ativo'),('t2','now','Outro','outro','Ativo'); INSERT INTO organizations VALUES ('o1','t1','now','ROOT','Org','Empresa','AOA','Ativa'),('o2','t2','now','ROOT','Outra','Empresa','AOA','Ativa'); INSERT INTO employees VALUES ('e1','t1','now','001','Ana','Silva','o1','Analista','2026-01-01','Ativo'),('e2','t2','now','002','Eva','Lima','o2','Analista','2026-01-01','Ativo')");assert.throws(()=>db.exec("INSERT INTO payroll_adjustments VALUES ('bad','t1','e2','ADJ-X','Retroativo','Earning','2026-01','2026-02','AOA',100,'Diferença','Memória','Pendente','ana@test','now',NULL,NULL,NULL,NULL)"),/payroll adjustment outside tenant/);assert.throws(()=>db.exec("INSERT INTO payroll_adjustments VALUES ('bad2','t1','e1','ADJ-Y','Retroativo','Earning','2026-03','2026-02','AOA',100,'Diferença','Memória','Pendente','ana@test','now',NULL,NULL,NULL,NULL)"),/source period is after payment period/);
 db.exec("INSERT INTO payroll_adjustments VALUES ('a1','t1','e1','ADJ-1','Retroativo','Earning','2026-01','2026-02','AOA',100,'Diferença salarial','Memória validada','Pendente','ana@test','now',NULL,NULL,NULL,NULL); UPDATE payroll_adjustments SET status='Aprovado',decided_by='rh@test',decided_at='later' WHERE id='a1'");assert.throws(()=>db.exec("UPDATE payroll_adjustments SET amount_minor=101 WHERE id='a1'"),/invalid payroll adjustment transition/);assert.throws(()=>db.exec("DELETE FROM payroll_adjustments WHERE id='a1'"),/payroll adjustments cannot be deleted/);db.close();
});

test("demo portfolio installation marker is immutable and idempotent",()=>{
 const db=migratedDatabase();db.exec("INSERT INTO demo_portfolio_installations VALUES ('t1','DEMO-2026.1','admin@test','now',65)");
 assert.throws(()=>db.exec("INSERT INTO demo_portfolio_installations VALUES ('t1','DEMO-2026.1','admin@test','later',65)"),/UNIQUE constraint failed/);
 assert.throws(()=>db.exec("UPDATE demo_portfolio_installations SET record_count=66 WHERE tenant_id='t1'"),/installation is immutable/);
 assert.throws(()=>db.exec("DELETE FROM demo_portfolio_installations WHERE tenant_id='t1'"),/cannot be deleted/);db.close();
});
