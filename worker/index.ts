...
type SecurityContext={email:string;name:string;role:string;tenantId:string;permissions:Permission[]};
...
async function securityContext(request:Request,db:D1Database):Promise<SecurityContext|Response>{await ensureSetupSchema(db);const email=actorEmail(request);if(!email)return Response.json({error:"Autenticação necessária."},{status:401});let user=await db.prepare("SELECT * FROM platform_users WHERE tenant_id=? AND lower(email)=lower(?) AND status='Ativo'").bind(TENANT,email).first<Record<string,unknown>>();if(!user){const total=await db.prepare("SELECT COUNT(*) n FROM platform_users WHERE tenant_id=?").bind(TENANT).first<Record<string,unknown>>();if(Number(total?.n||0)===0){const name=request.headers.get("oai-authenticated-user-full-name")||"Administrador inicial",created=new Date().toISOString(),id=uid();await db.batch([db.prepare("INSERT INTO platform_users (id,tenant_id,created_at,name,email,role,organization_id,status) VALUES (?,?,?,?,?,?,?,?)").bind(id,TENANT,created,name,email,"Administrador",null,"Ativo"),db.prepare("INSERT INTO audit_events (id,tenant_id,created_at,action,entity_type,entity_id,actor,summary) VALUES (?,?,?,?,?,?,?,?)").bind(uid(),TENANT,created,"BOOTSTRAP","platformUser",id,email,"Administrador inicial associado ao tenant")]);user={id,name,email,role:"Administrador",tenant_id:TENANT}}else return Response.json({error:"O utilizador autenticado não possui membership ativa neste tenant."},{status:403})}const role=String(user.role),permissions=rolePermissions[role]||[];return{email,name:String(user.name||email),role,tenantId:String(user.tenant_id),permissions}}
...
async function payrollApi(request:Request,db:D1Database){
 ...
 if(body.type==="salaryProfile"){
  ...
  for(const p of profiles.results){const employeeId=String(p.employee_id),base=Number(p.base_minor),comps=await db.prepare("SELECT c.* FROM payroll_components c JOIN payroll_assignments a ON a.component_id=c.id WHERE a.tenant_id=? AND a.employee_id=? AND c.status='Ativo' ORDER BY c.calculation_order,c.code").bind(TENANT,employeeId).all<Record<string,unknown>>();let gross=base,deductions=0,employer=0;const inputs=[] as Record<string,unknown>[];for(const c of comps.results){const amount=c.method==="Fixed"?Number(c.value_minor):percentageAmount(base,Number(c.rate_bps));if(c.category==="Earning")gross+=amount;else if(c.category==="Deduction")deductions+=amount;else employer+=amount;inputs.push({code:c.code,category:c.category,method:c.method,amountMinor:amount})}const net=gross-deductions,snapshot=JSON.stringify({employeeId,baseMinor:base,components:inputs});const hash=await sha256(snapshot);grossTotal+=gross;dedTotal+=deductions;employerTotal+=employer;netTotal+=net;statements.push(db.prepare("INSERT INTO payroll_run_lines (id,tenant_id,run_id,employee_id,base_minor,gross_minor,deduction_minor,employer_minor,net_minor,calculation_hash,input_snapshot,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(uid(),TENANT,recordId,employeeId,base,gross,deductions,employer,net,hash,snapshot,created));}
  ...
  const run=await db.prepare("SELECT status,period FROM payroll_runs WHERE id=? AND tenant_id=?").bind(body.runId,TENANT).first<{status:string,period:string}>();if(!run)return Response.json({error:"Processamento não encontrado."},{status:404});const security=await securityContext(request,db);if(security instanceof Response)return security;if(run.status==="Rascunho"&&security.role!=="Recursos Humanos")return Response.json({error:"Segregação de funções: a validação do Payroll pertence a Recursos Humanos."},{status:403});if(["Validado","Aprovado"].includes(run.status)&&security.role!=="Administrador")return Response.json({error:"Segregação de funções: aprovação e fecho pertencem à Administração."},{status:403});const next:{[key:string]:string}={Rascunho:"Validado",Validado:"Aprovado",Aprovado:"Fechado"};if(!next[run.status])return Response.json({error:"O processamento já está fechado."},{status:409});await db.prepare("UPDATE payroll_runs SET status=?,closed_at=? WHERE id=? AND tenant_id=?").bind(next[run.status],next[run.status]==="Fechado"?created:null,body.runId,TENANT).run();summary=`Payroll ${run.period}: ${next[run.status]}`;action=next[run.status].toUpperCase();
 }else return Response.json({error:"Operação não suportada."},{status:400});
...
async function integrityApi(request:Request,db:D1Database){try{await ensureSetupSchema(db);if(request.method!=="GET")return Response.json({error:"Método não permitido."},{status:405});const rows=await Promise.all([
 db.prepare("SELECT COUNT(*) n FROM organizations WHERE tenant_id=? AND status='Ativa'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) n FROM platform_users WHERE tenant_id=? AND status='Ativo'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) n FROM employees e JOIN organizations o ON o.id=e.organization_id AND o.tenant_id=e.tenant_id WHERE e.tenant_id=? AND e.status='Ativo'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) n FROM dimension_members m JOIN financial_dimensions d ON d.id=m.dimension_id AND d.tenant_id=m.tenant_id WHERE m.tenant_id=? AND m.status='Ativo'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(DISTINCT scenario) n,COUNT(*) entries FROM performance_entries WHERE tenant_id=? AND scenario IN ('Actual','Budget')").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) n FROM payroll_runs WHERE tenant_id=? AND status='Fechado'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COALESCE(SUM(l.gross_minor+l.employer_minor),0) expected,COALESCE(SUM(w.total_minor),0) posted,COUNT(DISTINCT w.run_id) runs FROM payroll_run_lines l JOIN payroll_runs r ON r.id=l.run_id AND r.tenant_id=l.tenant_id LEFT JOIN workforce_cost_postings w ON w.run_line_id=l.id AND w.tenant_id=l.tenant_id WHERE l.tenant_id=? AND r.status='Fechado'").bind(TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT COALESCE(SUM(w.total_minor),0) posted,COALESCE((SELECT SUM(amount_minor) FROM performance_entries WHERE tenant_id=? AND scenario='Actual' AND source='Payroll' AND line_code='WORKFORCE'),0) actual FROM workforce_cost_postings w WHERE w.tenant_id=?").bind(TENANT,TENANT).first<Record<string,unknown>>(),
 db.prepare("SELECT * FROM management_reports WHERE tenant_id=? ORDER BY report_number DESC LIMIT 1").bind(TENANT).first<Record<string,unknown>>(),
