...
type Permission="setup:write"|"performance:write"|"payroll:write"|"workforce:write"|"reports:write"|"integrity:read";
type TenantOption={id:string;name:string;role:string};
type SecurityContext={email:string;name:string;role:string;tenantId:string;tenantName:string;permissions:Permission[];tenants:TenantOption[]};
const rolePermissions:Record<string,Permission[]>={Administrador:["setup:write","performance:write","payroll:write","workforce:write","reports:write","integrity:read"],Financeiro:["performance:write","workforce:write","reports:write","integrity:read"],"Recursos Humanos":["payroll:write"],Gestor:["reports:write"],Leitura:[]};
...
async function setupSnapshot(db: D1Database,tenantId:string) {
  const [organizations, users, employees, audit, dimensions, dimensionMembers] = await Promise.all([
    db.prepare("SELECT * FROM organizations WHERE tenant_id=? ORDER BY created_at").bind(tenantId).all(),
    db.prepare("SELECT * FROM platform_users WHERE tenant_id=? ORDER BY created_at").bind(tenantId).all(),
    db.prepare("SELECT * FROM employees WHERE tenant_id=? ORDER BY created_at DESC").bind(tenantId).all(),
    db.prepare("SELECT * FROM audit_events WHERE tenant_id=? ORDER BY created_at DESC LIMIT 8").bind(tenantId).all(),
...
async function setupApi(request: Request, db: D1Database,tenantId:string) {
  try {
    await ensureSetupSchema(db);
    if (request.method === "GET") return Response.json(await setupSnapshot(db,tenantId));
    if (request.method !== "POST") return Response.json({error:"Método não permitido."},{status:405});
...
async function performanceSnapshot(db:D1Database, url:URL,tenantId:string){
  ...
  const [entries,versions,organizations,members,summary,audit]=await Promise.all([
    db.prepare("SELECT e.*, o.name AS organization_name, m.name AS dimension_member_name FROM performance_entries e JOIN organizations o ON o.id=e.organization_id LEFT JOIN dimension_members m ON m.id=e.dimension_member_id WHERE e.tenant_id=? AND e.period=? AND e.currency=? AND (e.scenario='Actual' OR e.version_id=?) ORDER BY e.created_at DESC LIMIT 100").bind(tenantId,period,currency,version).all(),
    db.prepare("SELECT * FROM budget_versions WHERE tenant_id=? ORDER BY fiscal_year DESC, created_at DESC").bind(tenantId).all(),
    db.prepare("SELECT id,name,code,currency FROM organizations WHERE tenant_id=? AND status='Ativa' ORDER BY name").bind(tenantId).all(),
    ...
    db.prepare("SELECT COALESCE(SUM(CASE WHEN scenario='Actual' THEN amount_minor ELSE 0 END),0) AS actual_minor, COALESCE(SUM(CASE WHEN scenario='Budget' AND version_id=? THEN amount_minor ELSE 0 END),0) AS budget_minor FROM performance_entries WHERE tenant_id=? AND period=? AND currency=?").bind(version,tenantId,period,currency).first(),
    db.prepare("SELECT * FROM audit_events WHERE tenant_id=? AND entity_type IN ('performanceEntry','budgetVersion','approveBudget') ORDER BY created_at DESC LIMIT 6").bind(tenantId).all(),
  ]);
...
async function performanceApi(request:Request,db:D1Database,tenantId:string){
 try{await ensureSetupSchema(db);const url=new URL(request.url);if(request.method==="GET")return Response.json(await performanceSnapshot(db,url,tenantId));if(request.method!=="POST")return Response.json({error:"Método não permitido."},{status:405});
 ...
 if(body.type==="budgetVersion"){
   ...
   await db.prepare("UPDATE budget_versions SET status='Aprovado', approved_at=? WHERE id=? AND tenant_id=?").bind(created,body.versionId,tenantId).run();summary=`Versão orçamental ${version.name} aprovada`;entityType="approveBudget";
 }else return Response.json({error:"Operação não suportada."},{status:400});
...
async function payrollSnapshot(db:D1Database,tenantId:string){
 const [profiles,components,runs,employees,members,audit]=await Promise.all([
  db.prepare("SELECT p.*, e.first_name||' '||e.last_name AS employee_name, e.employee_number, m.name AS dimension_member_name FROM salary_profiles p JOIN employees e ON e.id=p.employee_id LEFT JOIN dimension_members m ON m.id=p.dimension_member_id WHERE p.tenant_id=? ORDER BY p.created_at DESC").bind(tenantId).all(),
  db.prepare("SELECT c.*, e.first_name||' '||e.last_name AS employee_name FROM payroll_components c LEFT JOIN payroll_assignments a ON a.component_id=c.id AND a.tenant_id=c.tenant_id LEFT JOIN employees e ON e.id=a.employee_id WHERE c.tenant_id=? ORDER BY c.calculation_order,c.code").bind(tenantId).all(),
  db.prepare("SELECT * FROM payroll_runs WHERE tenant_id=? ORDER BY period DESC,created_at DESC LIMIT 12").bind(tenantId).all(),
  db.prepare("SELECT id,employee_number,first_name,last_name FROM employees WHERE tenant_id=? AND status='Ativo' ORDER BY first_name,last_name").bind(tenantId).all(),
  db.prepare("SELECT m.id,m.name,m.code,d.name AS dimension_name FROM dimension_members m JOIN financial_dimensions d ON d.id=m.dimension_id WHERE m.tenant_id=? AND m.status='Ativo' ORDER BY d.name,m.code").bind(tenantId).all(),
  db.prepare("SELECT * FROM audit_events WHERE tenant_id=? AND entity_type IN ('salaryProfile','payrollComponent','payrollRun') ORDER BY created_at DESC LIMIT 6").bind(tenantId).all(),
...
async function payrollApi(request:Request,db:D1Database,tenantId:string){
 try{await ensureSetupSchema(db);if(request.method==="GET")return Response.json(await payrollSnapshot(db,tenantId));if(request.method!=="POST")return Response.json({error:"Método não permitido."},{status:405});
 ...
 if(body.type==="salaryProfile"){
  ...
  if(await db.prepare("SELECT id FROM payroll_runs WHERE tenant_id=? AND period=? AND currency=?").bind(tenantId,body.period,body.currency.toUpperCase()).first())return Response.json({error:"Já existe um processamento para este período e moeda."},{status:409});
  const profiles=await db.prepare("SELECT p.*,e.id AS employee_id FROM salary_profiles p JOIN employees e ON e.id=p.employee_id WHERE p.tenant_id=? AND p.status='Ativo' AND p.currency=? AND p.effective_from<=? AND (p.effective_to IS NULL OR p.effective_to>=?) AND e.status='Ativo'").bind(tenantId,body.currency.toUpperCase(),`${body.period}-31`,`${body.period}-01`).all<Record<string,unknown>>();if(!profiles.results.length)return Response.json({error:"Não existem perfis salariais elegíveis."},{status:409});
  ...
  const run=await db.prepare("SELECT status,period FROM payroll_runs WHERE id=? AND tenant_id=?").bind(body.runId,tenantId).first<{status:string,period:string}>();if(!run)return Response.json({error:"Processamento não encontrado."},{status:404});const security=await securityContext(request,db);if(security instanceof Response)return security;if(run.status==="Rascunho"&&security.role!=="Recursos Humanos")return Response.json({error:"Segregação de funções: a validação do Payroll pertence a Recursos Humanos."},{status:403});if(["Validado","Aprovado"].includes(run.status)&&security.role!=="Administrador")return Response.json({error:"Segregação de funções: aprovação e fecho pertencem à Administração."},{status:403});const next:{[key:string]:string}={Rascunho:"Validado",Validado:"Aprovado",Aprovado:"Fechado"};if(!next[run.status])return Response.json({error:"O processamento já está fechado."},{status:409});await db.prepare("UPDATE payroll_runs SET status=?,closed_at=? WHERE id=? AND tenant_id=?").bind(next[run.status],next[run.status]==="Fechado"?created:null,body.runId,tenantId).run();summary=`Payroll ${run.period}: ${next[run.status]}`;action=next[run.status].toUpperCase();
 }else return Response.json({error:"Operação não suportada."},{status:400});
 await db.prepare("INSERT INTO audit_events (id,tenant_id,action,entity_type,entity_id,actor,summary,created_at) VALUES (?,?,?,?,?,?,?,?)").bind(uid(),tenantId,action,body.type==="transitionPayrollRun"?"payrollRun":body.type,body.runId||recordId,actor,summary,created).run();return Response.json(await payrollSnapshot(db,tenantId),{status:201});
 }catch(error){return Response.json({error:error instanceof Error?error.message:"Erro de payroll."},{status:500})}
}

async function workforceSnapshot(db:D1Database,url:URL,tenantId:string){const period=url.searchParams.get("period")||"2026-08",currency=(url.searchParams.get("currency")||"AOA").toUpperCase(),version=url.searchParams.get("version")||"";const [runs,postings,versions,summary,audit]=await Promise.all([
 db.prepare("SELECT r.*,COUNT(w.id) AS posting_count FROM payroll_runs r LEFT JOIN workforce_cost_postings w ON w.run_id=r.id AND w.tenant_id=r.tenant_id WHERE r.tenant_id=? AND r.status='Fechado' GROUP BY r.id ORDER BY r.period DESC").bind(tenantId).all(),
 db.prepare("SELECT w.*,e.first_name||' '||e.last_name AS employee_name,e.employee_number,o.name AS organization_name,m.name AS dimension_member_name FROM workforce_cost_postings w JOIN employees e ON e.id=w.employee_id JOIN organizations o ON o.id=w.organization_id LEFT JOIN dimension_members m ON m.id=w.dimension_member_id WHERE w.tenant_id=? AND w.period=? AND w.currency=? ORDER BY employee_name").bind(tenantId,period,currency).all(),
 ...
 db.prepare("SELECT COALESCE((SELECT SUM(total_minor) FROM workforce_cost_postings WHERE tenant_id=? AND period=? AND currency=?),0) AS actual_minor,COALESCE((SELECT SUM(amount_minor) FROM performance_entries WHERE tenant_id=? AND period=? AND currency=? AND scenario='Budget' AND version_id=? AND line_code='WORKFORCE'),0) AS budget_minor").bind(tenantId,period,currency,tenantId,period,currency,version).first(),
 db.prepare("SELECT * FROM audit_events WHERE tenant_id=? AND entity_type='workforceCost' ORDER BY created_at DESC LIMIT 6").bind(tenantId).all(),
...
async function dashboardApi(request:Request,db:D1Database,tenantId:string){try{await ensureSetupSchema(db);const u=new URL(request.url),period=u.searchParams.get("period")||"2026-08",currency=(u.searchParams.get("currency")||"AOA").toUpperCase(),version=u.searchParams.get("version")||"";const [versions,totals,trend,drivers,entries,coverage,headcount,payroll]=await Promise.all([
 ...
 db.prepare("SELECT line_code,MAX(line_name) line_name,COALESCE(SUM(CASE WHEN scenario='Actual' THEN amount_minor ELSE 0 END),0) actual_minor,COALESCE(SUM(CASE WHEN scenario='Budget' AND version_id=? THEN amount_minor ELSE 0 END),0) budget_minor FROM performance_entries WHERE tenant_id=? AND period=? AND currency=? GROUP BY line_code ORDER BY line_code").bind(version,tenantId,period,currency).all<Record<string,unknown>>(),
 db.prepare("SELECT p.*,o.name organization_name,m.name dimension_member_name FROM performance_entries p JOIN organizations o ON o.id=p.organization_id LEFT JOIN dimension_members m ON m.id=p.dimension_member_id WHERE p.tenant_id=? AND p.period=? AND p.currency=? AND (p.scenario='Actual' OR (p.scenario='Budget' AND p.version_id=?)) ORDER BY p.created_at DESC").bind(tenantId,period,currency,version).all(),
 db.prepare("SELECT COUNT(DISTINCT organization_id) organizations,COUNT(DISTINCT source) sources,MAX(created_at) latest_at FROM performance_entries WHERE tenant_id=? AND period=? AND currency=?").bind(tenantId,period,currency).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) total FROM employees WHERE tenant_id=? AND status='Ativo'").bind(tenantId).first<Record<string,unknown>>(),
 db.prepare("SELECT period,status FROM payroll_runs WHERE tenant_id=? AND currency=? AND period<=? ORDER BY period DESC,created_at DESC LIMIT 1").bind(tenantId,currency,period).first(),
...
async function managementReportApi(request:Request,db:D1Database,tenantId:string){try{await ensureSetupSchema(db);const u=new URL(request.url);if(request.method==="GET"){const id=u.searchParams.get("id");if(id){const row=await db.prepare("SELECT * FROM management_reports WHERE tenant_id=? AND id=?").bind(tenantId,id).first<Record<string,unknown>>();if(!row)return Response.json({error:"Relatório não encontrado."},{status:404});return Response.json({...row,payload:JSON.parse(String(row.payload_json))})}const [reports,versions]=await Promise.all([db.prepare("SELECT id,report_number,title,template,period,currency,status,input_hash,created_by,created_at FROM management_reports WHERE tenant_id=? ORDER BY report_number DESC LIMIT 20").bind(tenantId).all(),db.prepare("SELECT id,name,status,fiscal_year FROM budget_versions WHERE tenant_id=? ORDER BY fiscal_year DESC,created_at DESC").bind(tenantId).all()]);return Response.json({reports:reports.results,versions:versions.results})}if(request.method!=="POST")return Response.json({error:"Método não permitido."},{status:405});const b=await request.json() as Record<string,string>,period=b.period||"",currency=(b.currency||"").toUpperCase(),version=b.versionId||"";if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(period)||!/^[A-Z]{3}$/.test(currency)||!b.title?.trim())return Response.json({error:"Título, período e moeda válidos são obrigatórios."},{status:400});const [totals,drivers,coverage,versionRow,last]=await Promise.all([
db.prepare("SELECT COALESCE(SUM(CASE WHEN scenario='Actual' THEN amount_minor ELSE 0 END),0) actual,COALESCE(SUM(CASE WHEN scenario='Budget' AND version_id=? THEN amount_minor ELSE 0 END),0) budget,COALESCE(SUM(CASE WHEN scenario='Actual' AND line_code='WORKFORCE' THEN amount_minor ELSE 0 END),0) workforce FROM performance_entries WHERE tenant_id=? AND period=? AND currency=?").bind(version,tenantId,period,currency).first<Record<string,unknown>>(),
...
db.prepare("SELECT COUNT(*) entries,COUNT(DISTINCT organization_id) organizations,COUNT(DISTINCT source) sources,MAX(created_at) latest_at FROM performance_entries WHERE tenant_id=? AND period=? AND currency=?").bind(tenantId,period,currency).first<Record<string,unknown>>(),
...
async function integrityApi(request:Request,db:D1Database,tenantId:string){try{await ensureSetupSchema(db);if(request.method!=="GET")return Response.json({error:"Método não permitido."},{status:405});const rows=await Promise.all([
 ...
 db.prepare("SELECT COUNT(DISTINCT scenario) n,COUNT(*) entries FROM performance_entries WHERE tenant_id=? AND scenario IN ('Actual','Budget')").bind(tenantId).first<Record<string,unknown>>(),
 db.prepare("SELECT COUNT(*) n FROM payroll_runs WHERE tenant_id=? AND status='Fechado'").bind(tenantId).first<Record<string,unknown>>(),
 db.prepare("SELECT COALESCE(SUM(l.gross_minor+l.employer_minor),0) expected,COALESCE(SUM(w.total_minor),0) posted,COUNT(DISTINCT w.run_id) runs FROM payroll_run_lines l JOIN payroll_runs r ON r.id=l.run_id AND r.tenant_id=l.tenant_id LEFT JOIN workforce_cost_postings w ON w.run_line_id=l.id AND w.tenant_id=l.tenant_id WHERE l.tenant_id=? AND r.status='Fechado'").bind(tenantId).first<Record<string,unknown>>(),
 db.prepare("SELECT COALESCE(SUM(w.total_minor),0) posted,COALESCE((SELECT SUM(amount_minor) FROM performance_entries WHERE tenant_id=? AND scenario='Actual' AND source='Payroll' AND line_code='WORKFORCE'),0) actual FROM workforce_cost_postings w WHERE w.tenant_id=?").bind(tenantId,tenantId).first<Record<string,unknown>>(),
 db.prepare("SELECT * FROM management_reports WHERE tenant_id=? ORDER BY report_number DESC LIMIT 1").bind(tenantId).first<Record<string,unknown>>(),
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (apiPath.startsWith("/api/")) {
      ...
      if (apiPath === "/api/workforce") return workforceApi(request, env.DB,tenantId);
      if (apiPath === "/api/dashboard") return dashboardApi(request, env.DB,tenantId);
      if (apiPath === "/api/management-reports") return managementReportApi(request, env.DB,tenantId);
      if (apiPath === "/api/integrity") return integrityApi(request, env.DB,tenantId);
    }
